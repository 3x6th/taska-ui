...
| [TAS-242](https://jira.ozero.dev/browse/TAS-242) | The issue panel's five sections start their reads with the issue instead of after it; a skeleton replaces the "Loading issue" line | In Progress | frontend, filed 2026-09-23 from the TAS-210 loading analysis, epic TAS-210. The second round was self-inflicted by the `if (!issue)` guard. The board's duplicate project read is deliberately left for TAS-212/TAS-202 |
