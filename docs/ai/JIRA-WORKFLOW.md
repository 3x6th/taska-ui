...
| [TAS-226](https://jira.ozero.dev/browse/TAS-226) | The issue panel offered a VIEWER two writes issue-service refuses — Watch, and being chosen as assignee — and a `200` with no `currentUserRole` silently became VIEWER | In Progress | filed 2026-09-16 from three `release-reviewer` notes on TAS-224, which graduated out of `BACKLOG.md`; in progress on `fix/TAS-226-viewer-gating`. Server rules read at `develop` `1cfe4d7`: `watch-issue-roles` and `assign-issue-roles` are both `ADMIN,MEMBER`, and the second is checked for the assignee as well as the actor |
