...
| [TAS-221](https://jira.ozero.dev/browse/TAS-221) | Avatar confirm trusts any `objectKey`: one user can delete another's avatar, and a same-key re-confirm destroys one's own | To Do | backend ask, High, filed from TAS-220 on 2026-09-14 from a read of `develop` `368ae77` — to be fixed before PR #150 deploys |
...
| [TAS-129](https://jira.ozero.dev/browse/TAS-129) | [TAS-220](https://jira.ozero.dev/browse/TAS-220) on the stand, not in the repository | Avatar upload through the gateway (presigned URL to MinIO), on top of TAS-128. **Done in Jira and merged** (backend PR #150, `develop` `368ae77`, 2026-09-14), but **not deployed**: the four routes answered the static-resource 404 at 11:14 UTC that day, and the owner confirmed the deploy did not happen. It merged with three defects filed as their own tickets — [TAS-221](https://jira.ozero.dev/browse/TAS-221) (confirm trusts any `objectKey`, High, before the deploy), [TAS-222](https://jira.ozero.dev/browse/TAS-222) (5 MB declared, 2 MB enforced, 3 MB answers 500) and [TAS-223](https://jira.ozero.dev/browse/TAS-223) (its own acceptance criterion: no avatar on `GET /users/me`). Still the base under the computed avatar colour in `API-DIVERGENCE.md`'s colour entry. |
