...
| [TAS-210](https://jira.ozero.dev/browse/TAS-210) | **Epic** — API for the screens: every screen complete in one round of requests instead of 30, 9 and 6 requests over two rounds | To Do | filed 2026-09-10 as one task, made an epic 2026-09-11 at the owner's asking; priority `Highest` set by the owner. Children TAS-211…218, TAS-240, TAS-241; TAS-202, TAS-137, TAS-204, TAS-184 and TAS-207 linked into it. **Rewritten 2026-09-23** after the owner asked about `/v2`: no version prefix, the expand-contract rule is written into the epic, and the target became one round per screen rather than one request — see the direction note below |
