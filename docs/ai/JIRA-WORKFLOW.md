...
| [TAS-220](https://jira.ozero.dev/browse/TAS-220) | The current user's avatar — upload, replace, delete, and drawing it wherever initials are today (backend PR #150) | In Progress | filed 2026-09-12 and written against backend PR #150's head `12e909d42dcc`, which merged unchanged into `develop` (`368ae77`) on 2026-09-14 — the pin is retired and the snapshot refreshed. The routes were not deployed at 11:14 UTC and were at 13:53 UTC; against the undeployed gateway the menu offered no photo controls and asked once per page load. `api-contract-guard` and `art-director` each returned two and three blockers on the first submission, fixed in the second pass; the backend defects that turned up are filed as TAS-221, TAS-222 and TAS-223, and PR #152's missing member avatar is on TAS-137 |
