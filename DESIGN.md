...
5. **Ничего лишнего.** Нет декоративных иллюстраций, эмодзи, градиентных фонов (два
   исключения: мягкое radial-свечение на экране логина и маскот-кубик на экране
   Not found — §4.18).
...
- **Маскот** — вторая (и последняя) разрешённая иллюстрация после свечения логина:
  расплавившийся 3D-кубик Taska с глазами-крестиками и высунутым языком,
  `src/assets/mascot-not-found.webp` (720×346, прозрачный фон, ~48 КБ).
  Сгенерирован владельцем во внешней модели (в исходном PNG — подписанный
  C2PA-манифест: `gpt-image`, `trainedAlgorithmicMedia`), затем обрезан и
  пережат для фронта. Исходный рендер с манифестом —
  `docs/design/mascot-not-found-source.png`; манифест намеренно не срезан,
  поэтому файл весит 2.2 МБ. Показывать через `<img>`
  шириной 260 (`max-width:78vw`, `height:auto`), `alt=""` — картинка
  декоративная, весь смысл несут заголовок и текст.
  Собственной тени не нужно: лужа с каплями в самой картинке уже сажает
  фигуру на плоскость.
- **Ограничение растрового маскота:** фиолетовый цвет запечён в пиксели и не
  следует за `--accent` (§2.1 разрешает менять акцент). Смена акцента требует
  перерисовки ассета — это единственное место в UI, где цвет не токенизирован.
  Текущий рендер и на дефолтном акценте не совпадает точно: тело маскота — OKLCH
  H≈287 против H=277 у `#4f46e5` (светлота почти равна, 0.514 против 0.511),
  поэтому оно читается как тот же материал, а не как другой цвет. Кто будет
  перерисовывать — целься в H 277 и получишь точное попадание в семью.
