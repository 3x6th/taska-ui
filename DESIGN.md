...
**Время в этом разделе — UTC, и поле обязано это сказать.** Таблица печатает то,
что отдал сервер, то есть ISO с `Z`; значит датапикер читается как UTC, а не как
локальные часы, и подписан «UTC». Иначе админ, глядя на колонку с `08:00:00Z`,
вводит 08:00 и получает фильтр на три часа раньше — цифры, которые он набрал, и
цифры, по которым идёт отбор, разъезжаются молча. На проводе — полный ISO-8601
