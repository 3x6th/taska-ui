...
- строка кликабельна и ведёт на `…/:id` — карточку строки. Кликабельна она
  только там, где каталог назвал primary key **и** его тип — `uuid`: гейтвей
  принимает `id` именно как UUID, поэтому строку с числовым или текстовым ключом
  адресовать нечем. Такая строка не кликабельна и не притворяется — ссылка,
  которая гарантированно приводит к отказу, хуже её отсутствия.
...
где его опции уже нет.

Для временной колонки значение вводится датапикером, а не свободным текстом, и
уходит полным ISO-8601 со смещением (`2026-01-01T00:00:00Z`). Сервер парсит его
строго: `2026-01-01` и `2026-01-01T00:00` — уже 400. Формат — это работа формы,
а не то, что админ обязан помнить; свободное поле здесь означало бы, что
правильный ввод угадывают.
