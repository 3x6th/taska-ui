...
входа пользователь возвращается именно туда, а не в общий список проектов, и на
форме видит короткое объяснение, почему его сюда прислали. Голый заход на `/`
(редирект в `/projects`) помечается как «открыл приложение», а не «просил
страницу», и объяснения не показывает. Зеркальное правило: посетителя, у
