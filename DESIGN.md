...
**Приватные маршруты.** `/projects`, `…/board` и `…/issues/:issueId` открываются
только с сессией: гвард спрашивает у `TaskaApi` синхронное «есть ли у клиента
учётные данные» и без них отдаёт `/login`, запомнив запрошенный адрес — после
входа пользователь возвращается именно туда, а не в общий список проектов.
