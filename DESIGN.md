...
уже как эта страница и после входа в неё же и возвращает; отдельного случая ему
не нужно. Ничего дополнительного форма при этом не объясняет: форма входа после
клика по закрытой ссылке объясняет себя сама. Зеркальное правило: посетителя, у
которого сессия уже есть, `/login` отправляет в `/projects`. `/invite` остаётся
публичным намеренно — инвайт принимают и будучи залогиненным под другим
аккаунтом; `*` (Page not found) тоже вне гварда: несуществующий адрес должен
отвечать «страницы нет», а не формой входа, которая намекала бы, что страница
существует и просто закрыта (§4.18). Наличие сессии — не право доступа: сервер
остаётся авторитетом (§5.7).
