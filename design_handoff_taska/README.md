...
> **Замороженный документ.** Это дизайн в том виде, в каком он был передан в разработку.
> Он не обновляется вслед за кодом. Актуальное состояние системы — `DESIGN.md` в корне
> репозитория; при расхождении прав `DESIGN.md`.
>
> Что здесь всё ещё авторитетно: `Taska.dc.html` как эталон внешнего вида экранов.
>
> `api-gateway-rest-draft.md` — **исторический черновик**, написанный до того, как
> появился гейтвей. Актуальный контракт — `openapi.yml` в репозитории бэкенда
> (снапшот: `docs/contract/openapi.yml`); аудировать код по черновику нельзя.
>
> Чего здесь нет, потому что появилось позже: комментарии к задачам, меню профиля и выход
> из аккаунта, режимы API `mock`/`rest`/`hybrid`. Всё это описано в `DESIGN.md`.
